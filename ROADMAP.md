...
- *Shipped (2026-08-27, ADR 0015):* **The joint multi-tenant submission
  index.** Candidate #13, built to the shape the plan itself specified: a table
  of contents over N already-signed packets, never a merged custody chain. It
  serves the organizer `campaign` cannot, the one holding packets rather than
  keys. Each row binds its member by that packet's own `bundle.json` SHA-256,
  and `habitable joint check` re-derives every recorded claim from the packets,
  so the index is never a trust root. `packet_version` stays 4 and the verifier
  is untouched. What it does not do is stated in the file itself: the members
