...
| RR-08 | **Storage-footprint UX on low-end devices** — show case size; safe offload of sealed originals without breaking the chain | DU1 | P1 | M | [P1][P2] · **[NET-NEW]** (also feedback R-03) · ✅ Implemented — case size + per-capture breakdown (#320/#325), `offload`/`restore` to external encrypted storage with the chain extended rather than rewritten (#296). Remaining: the web app shows the case total but not the per-capture breakdown or the offload action. |
