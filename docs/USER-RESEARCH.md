...
- **Values today:** the app is genuinely bilingual (EN/ES ships in v1, axe-gated in both languages); the photo-and-note flow is familiar; **no account to create**; the offline demo made sense; location is stripped from shared copies by default.
...
- **Values today:** the *entire premise* — no server, nothing to subpoena from the project, end-to-end encryption at rest and in sync, a duress-safe open state, location stripped from shared copies — is why he'd consider it at all; AGPL closes the hosted-service loophole.
...
- **Values today:** the packet's plain-language "what this proves / what it does not" cover section (now rendered at the top of `packet.html` and the PDF) speaks to the bench; location is stripped from shared copies, reducing PII-at-filing worries; the accessible HTML packet is readable.
