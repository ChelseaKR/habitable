...
| Read the authority from a vault the organizer happens to have | Rejected: it would make the command's behavior depend on whether an unrelated vault exists, and it would quietly make a network call the organizer did not ask for. |
