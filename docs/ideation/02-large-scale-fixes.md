...
**Status.** Done (2026-07-02). `resolve_deferred` and `retimestamp_all` now take
`extra_tsas` and are wired through the CLI (`resolve`, `retimestamp`) and the app server;
a resolved deferred capture is stamped against every redundant authority (mirroring
`_stamp_additional`), and `retimestamp_all` threads each extra authority into the archive
chain. The deferred queue did **not** need to persist the authority set — authorities are
resolved from `config.timestamp_authorities` at stamp time, exactly like the online path,
so `vault.py` was untouched. Note: because the packet verifier (`verify_archive_chain`)
requires a strictly linear archive chain, the redundant re-timestamps thread over the
previous link (each extra authority co-signs the chain forward) rather than being stored as
independent archives of the additional tokens. Covered by `tests/test_vault_capture.py`
(offline→resolve carries ≥2 authorities; an unreachable extra is skipped) and
`tests/test_archive.py` (threaded archive chain still verifies).
