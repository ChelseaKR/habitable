...
- **Health:** `GET /healthz` returns `200` with aggregate passthrough, retained-state,
  capacity-rejection, and rejected-journal counts, plus `startup_replay`; the container
  ships a `HEALTHCHECK` that polls it. These metrics contain no room id, token, or body
  data. `status` is `ok` only when the relay verified what it is reporting: with the
  journal refused or partly refused it is `degraded`, and the live-state counts describe
  process memory, never the persistence disk. Read `rooms: 0` together with
  `startup_replay` — `incomplete` means "did not look", not "holding nothing".
- **Readiness:** `GET /readyz` returns `503` when `startup_replay` is `incomplete`, i.e.
  an unknown amount of at-rest ciphertext was never read. That is deliberate: a relay
  that cannot see its own retained state should not be routed traffic while peers
  silently stop converging. Recovery is a manual, documented operator step — see
  [`relay-operator-self-audit.md`](relay-operator-self-audit.md) §4.8.
...
- **Opt-in persistence:** keep one local relay process per persistence directory. Startup
  ...
  before journal admission. An over-allowance directory fails closed — visibly:
  `/healthz` reports `startup_replay: incomplete` and `/readyz` refuses, because the
  journals stay on disk unloaded and uncounted until an operator acts (§4.8 of the
  self-audit). Unterminated append
...
- **Memory sizing:** 512 MiB is the retained-ciphertext ceiling, not a whole-process RSS
  ...
  retained-state cap.
- **Logs:** request lines remain off by default. Lifecycle lines, one fixed-vocabulary
  aggregate startup-replay line (a `warning` when degraded, an `error` when incomplete),
  and a fixed `relay request handler failed` event for unexpected faults may appear. When
  request logging is on, each line records the status the relay *actually wrote* and a
  `response` field of `complete`/`partial`/`none`; a request that returned nothing carries
  no `status` at all rather than a fabricated `200`. Expected resets/broken pipes are silent. The override replaces the
