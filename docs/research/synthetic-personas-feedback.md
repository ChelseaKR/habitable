...
| R-03 | **Done on the CLI.** Storage-footprint UX on low-end devices. Case size is shown, measured separately from the projected export copy, with the doubling documented (`Vault.storage_footprint`, `status --storage` with a per-capture breakdown). The safe offload path now exists: `habitable offload`/`habitable restore` move a sealed original to external encrypted storage and leave the content hash, the tokens and the chain of custody behind (issue #296, RR-08). **Remaining:** the web app shows the case total but neither the per-capture breakdown nor the offload action. | P-01 | high | M | C | partial | clean |
