...
| R-03 | **Partly done.** Storage-footprint UX on low-end devices. Shipped: case size shown, measured separately from the projected export copy, and the doubling documented (`Vault.storage_footprint`, `status` storage line, app + docs). **Not built: the safe offload path** — nothing in `src/` moves a sealed original off the device, so a tenant with a full phone has no supported way to reclaim space. That is issue #296, and `RESEARCH-ROADMAP.md` still carries it as RR-08 (P1, net-new). | P-01 | high | M | C | partial | clean |
